...
def update_banlist(edo_path):
    if open_button['text'] != "Select Edopro folder":
        ...
        warning = tkinter.Label(window, text = "Select Edopro path!", font=("Arial", 13), fg='#f00')
        warning.pack()

def update_mappings(edo_path):
    if open_button['text'] != "Select Edopro folder":
        if True:#if all files found
            print("\nupdating from: " + open_button['text'])
            list_name = "banlist.lflist.conf"
...
mappings_button = tkinter.Button(window, text="Update mappings", cursor="hand2", font=("Arial", 16), command=lambda: update_mappings(open_button['text']))
